...
export function ChatPage({
  ...
  // 新对话页面状态
  const [newChatAgentId, setNewChatAgentId] = useState('matlab-default');
  const [newChatCwd, setNewChatCwd] = useState('D:\\MATLAB_Workspace');
  ...
  const handleSend = useCallback((message: string) => {
    if (!currentSession) {
      ...
      onSendMessage(message, {
        ...
        cwd: newChatCwd || 'D:\\MATLAB_Workspace',
        permissionMode: permissionMode,
      }, (path) => {
        // 重置新对话选项
        setNewChatAgentId('matlab-default');
        setNewChatCwd('D:\\MATLAB_Workspace');
        navigate(path);
      });
    } else {
      onSendMessage(message);
