@echo off
REM ensure-running.bat - MATLAB Agent 确保服务运行脚本 (v5.2)
REM 用途: AI agent 调用此脚本，确保 MATLAB Agent 服务器在后台运行
REM 返回: 退出码 0 = 服务可用, 1 = 不可用
REM 用法: cmd /c "C:\Users\YOUR_USERNAME\.workbuddy\skills\matlab-agent\app\ensure-running.bat"

chcp 65001 >nul 2>&1
cd /d "%~dp0"

REM Step 1: 检查服务是否已在运行（用 PowerShell 检测）
REM 注意: 使用 2>nul 而不是 >nul 2>&1，避免 cmd /c 调用时的输入重定向错误
