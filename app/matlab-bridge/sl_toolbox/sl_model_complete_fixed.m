...
%   assignin('base', ['model_completed_' model_safe], true)
    ...
    mustPassChecks = {'unconnected', 'paramAudit'};  % [v12.0] +paramAudit (PM-02 FIX)
    if ~isSubPath
    ...
    if ~paramAuditResult.passed
        ...
        % [v12.0] paramAudit is now must-pass (PM-02 FIX)
        % Block if >20% of parameterized blocks have issues
        _pa_details = paramAuditResult.details;
        _pa_total = 0;
        if isfield(_pa_details, 'totalBlocks')
            _pa_total = _pa_details.totalBlocks;
        end
        _pa_issues = 0;
        if isfield(_pa_details, 'blocksWithIssues')
            _pa_issues = _pa_details.blocksWithIssues;
        end
        if _pa_total > 0 && _pa_issues / _pa_total > 0.2
            mustPassPassed = false;
        end
    end
