...
### 50. sl_delete_block — 全生命周期删除 API (v30)

#### 功能
核心删除 + 参数清理(signalLogging/callbacks/paramRegistry) + LineChildren递归 + preserveShell。替代旧版 `sl_delete_safe`。
...
| blockPath | string | ✅ | 待删除块的完整路径 |
| reason | string | ❌ | 删除原因（记录用） |
| cleanupParams | logical | ❌ | 是否清理参数注册表（默认 true） |
| cleanupSignalLogging | logical | ❌ | 是否清理信号日志（默认 true） |
| cleanupCallbacks | logical | ❌ | 是否清理回调（默认 true） |
| preserveShell | logical | ❌ | 保留子系统外壳（design_suspect 回滚场景，默认 false） |

#### 返回结构
```json
{
  "status": "ok",
  "blockPath": "model/Subsystem/Block",
  "deleted": true,
  "linesRemoved": 3,
  "paramsCleaned": ["Gain", "Param1"],
  "callbacksCleaned": ["DeleteFcn"],
  "preserveShell": false,
  "message": "Block deleted with full lifecycle cleanup"
}
```

#### 执行顺序（safety-critical）
1. Model load validation
2. Block existence validation
3. Record PortHandles connected lines
4. Cleanup params/signalLogging/callbacks BEFORE delete
5. preserveShell or delete_block
6. Verify deletion
7. LineChildren recursive orphaned line cleanup
8. Return result

#### Gate_DELETE_APPROVAL 触发
- 子系统级删除（2段路径，如 `model/Subsystem`）→ 返回 `pending_approval` + `approvalToken`
...
### 51. sl_delete_approval — 子系统删除影响分析 (v30)

#### 功能
子系统删除前的影响分析：子节点+下游依赖+连线统计。独立于 Bridge，纯 MATLAB 业务逻辑。
...
|------|------|:----:|------|
| modelName | string | ✅ | 模型名称 |
| subsystemPath | string | ✅ | 待删除子系统路径 |

#### 返回结构
```json
{
  "subsystemsToDelete": ["model/SubA", "model/SubA/Child1"],
  "childBlocks": 12,
  "downstreamSubsystems": ["model/SubB"],
  "totalAffectedLines": 8
}
```

#### 使用示例
```json
POST /api/matlab/sl/delete_approval
{"modelName": "Quadrotor_ADRC", "subsystemPath": "Quadrotor_ADRC/Reference_Generator"}
```
→ AI 将影响报告通过 `AskUserQuestion` 展示给用户，获取 approvalToken 后调用 `sl_delete_block`。

---

### 52. sl_retry_plan — 自动重试计划生成 (v30)

#### 功能
从 `sl_model_complete` 失败结果自动生成重试计划 + 断路器（3次同错误→升级 design_suspect）。
...
|------|------|:----:|------|
| modelName | string | ✅ | 模型名称 |
| subPath | string | ✅ | 子系统路径 |
| completeResult | struct | ✅ | `sl_model_complete` 返回的失败结果 |
| retryState | struct | ❌ | Bridge 维护的重试状态（含 retry_count, repeated_failure_count 等） |
...
```json
{
  ...
  "steps": ["Fix unconnected port Out1", "Add missing line from Gain1 to Sum1"],
  "remainingRetries": 7,
  "message": "Auto-generated fix plan",
  "autoGenerated": true
}
```

#### failureType 判定逻辑
- `implementation_error`（默认）: 连线/端口/布局等构建级问题
- `design_suspect`: 满足以下任一条件自动升级：
  - 连续3次同类型失败（断路器触发）
  - `completeResult.failedChecks` 含 paramAudit/blockPlan/compilation 失败

#### Gate_RETRY 状态机
- 10轮/子系统上限: 5轮实现修复 + 5轮设计回退
- 断路器: 连续3次同类型失败 → `implementation_error` 升级为 `design_suspect`
...
### 53. sl_rigor_score — 四维工程严谨性评分 (v12→v30)

#### 功能
领域无关的工程严谨性评分引擎。四维加权评分: 完整性(0.30) + 自洽性(0.35) + 可追溯性(0.20) + 可证明性(0.15)。阈值 0.65。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| microFramework | struct | ✅ | `sl_micro_design` 输出的微框架结构体 |
...
```json
{
  ...
  "weakestScore": 0.60,
  "fixHints": ["Add derivedFrom references for block Gain1"],
  "passed": true,
  "threshold": 0.65
}
```
...
#### 零动态子系统处理 (v26 FIX R1)
对纯代数/参数型子系统（无状态、无积分器、无导数算子），使用基于参数质量的评分模型。

---

### 54. sl_rigor_utils — 符号分析工具集 (v12→v30)

#### 功能
Rigor Score 引擎的 9 个辅助函数。纯形式逻辑，零领域知识。
...
| Action | 输入 | 输出 | 说明 |
|--------|------|------|------|
| `extract_variables` | equation strings | cell array | 提取方程中的变量名 |
| `count_derivative_operators` | equation strings | scalar | 统计 d/dt 算子数 |
| `count_integrators` | blockPlan | scalar | 统计 Integrator 块数 |
| `check_operation_coverage` | equations, blockPlan | struct | 方程算子→块类型映射覆盖检查 |
| `check_parameter_usage` | params, equations | struct | 参数引用图检查 |
| `check_no_orphan_blocks` | blockPlan, equations | struct | 检测无对应方程的孤立块 |
| `get_defined_variables` | microFramework | cell array | 收集所有已定义变量 |
| `split_equation` | equation string | struct | LHS/RHS/算子拆分 |
| `extract_operators` | equation string | cell array | 提取数学算子 |

#### 返回结构
```json
{"status": "ok", "action": "count_integrators", "data": 3}
```

---

### 55. sl_param_registry — 物理参数注册与验证 (v12→v30)

#### 功能
物理参数标准化基础设施（值+单位+范围+来源），预置 quadrotor/pendulum/RLC 模板。
...
| Action | 参数 | 说明 |
|--------|------|------|
| `init` | domain | 初始化域（如 'quadrotor'） |
| `define` | name, struct{value,unit,range,source} | 定义参数 |
| `validate` | — | 验证当前模型所有参数 |
| `validate_value` | name, value | 验证单参数值在范围内 |
| `list` | — | 列出所有已注册参数 |
| `export` | format | 导出参数表 |
| `get_template` | domain | 获取域模板（quadrotor/pendulum/RLC） |
| `remove` | blockPath | 删除与块关联的参数（sl_delete_block 内部调用） |

#### 使用示例
```matlab
sl_param_registry('init', 'quadrotor')
sl_param_registry('define', 'mass', struct('value', 1.5, 'unit', 'kg', 'range', [0.1, 50], 'source', 'datasheet'))
sl_param_registry('validate')
```

---

### 56. sl_micro_approve_guard — 审批前置检查 (v12→v30)

#### 功能
`sl_micro_approve` 执行前的双重守卫：Gate_APPROVE_NO_REVIEW（review 必须通过）+ Gate_CONTENT_DEPTH（rigor ≥ 0.65）。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| subsystemName | string | ✅ | 子系统标识符 |
| microFramework | struct | ✅ | `sl_micro_design` 输出 |
| reviewResult | struct | ✅ | `sl_micro_review` 输出 |
...
```json
{
  "passed": true,
  "checks": [
    {"guard": "APPROVE_NO_REVIEW", "passed": true, "issue": "", "confidence": 0.9},
    {"guard": "CONTENT_DEPTH", "passed": true, "issue": "", "rigorScore": 0.72, "threshold": 0.65}
  ],
  "message": "All guards passed — safe to approve",
  "subsystemName": "TD_X"
}
```

#### 守卫逻辑
1. **Guard 1 — APPROVE_NO_REVIEW**: `reviewResult.passed === true` 否则拦截
2. **Guard 2 — CONTENT_DEPTH**: `sl_rigor_score(microFramework).score >= 0.65` 否则拦截并返回 fixHints
